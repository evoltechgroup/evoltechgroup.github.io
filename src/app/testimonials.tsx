  ...
  return (
    <section className="relative w-full bg-[#eaf6ff] py-20 flex flex-col items-center">
      <div className="max-w-3xl w-full mx-auto flex flex-col items-center">
        <h2 className="text-4xl font-bold text-center mb-2 text-[#0B0F2B]">Testimonials</h2>
        <div className="flex items-center gap-2 mb-2">
          <span className="text-[#0B0F2B] font-medium text-lg">Happy</span>
          <span className="inline-block align-middle">
            <img src="/assets/icons/smile-emoji.png" alt=":)" className="w-7 h-7 " />
          </span>
          <span className="text-[#0B0F2B] font-medium text-lg">customers about us</span>
        </div>
        {/* Decorative Arrow */}
        ...
        <div className="relative w-full flex justify-center">
          <button onClick={prev} aria-label="Previous" className="absolute left-0 top-1/2 -translate-y-1/2 z-10 p-2 focus:outline-none">
          ...
          <div className="bg-white rounded-2xl shadow-lg px-8 py-8 max-w-2xl w-full flex flex-col items-center relative transition-all duration-500">
            <img src="/assets/icons/left-quote.png" alt="quote left" className="absolute left-4 top-4 w-6 h-6 opacity-40" />
            <img src="/assets/icons/right-quote.png" alt="quote right" className="absolute right-4 bottom-4 w-6 h-6 opacity-40" />
            <p className="text-[#222] text-center text-base whitespace-pre-line min-h-[160px]">{t.text}</p>
          </div>
          <button onClick={next} aria-label="Next" className="absolute right-0 top-1/2 -translate-y-1/2 z-10 p-2 focus:outline-none">
            <span className="sr-only">Next</span>
          </button>
        </div>
        <div className="mt-6 text-center">
          <div className="font-bold text-[#0B0F2B] text-lg">{t.author}</div>
          <button className="bg-yellow-400 text-[#0B0F2B] px-4 py-1 rounded-full font-semibold text-xs mt-2 cursor-default">{t.company}</button>
        </div>
        {/* Slider dots */}
        <div className="flex gap-2 justify-center mt-4">
          {testimonials.map((_, i) => (
            <span
              key={i}
              className={`w-2 h-2 rounded-full ${i === index ? 'bg-[#0B0F2B]' : 'bg-[#0B0F2B]/20'} inline-block`}
            />
          ))}
        </div>
      </div>
